...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <script>
        ...
        $(function() {
            $.validator.setDefaults({
                ...
                submitHandler: function(form) {
                    ...
                    var url;
                    if (todo == "update") {
                        url = "<?php echo base_url('master/service/medic/update'); ?>";
                    } else {
        ...
        function editService(id) {
            $('.overlay-loading').show();
            $.ajax({
                url: "<?php echo base_url('master/service/medic/detail/'); ?>",
                type: "POST",
                data: {
                    id: id
                },
        ...
        function deleteService(id) {
            bootbox.confirm({
                title: "Hapus Tindakan Medikal",
                message: "Apakah kamu yakin untuk menghapus tindakan medikal ini? Aksi ini tidak bisa di kembalikan",
                buttons: {
                    cancel: {
                ...
                callback: function(result) {
                    if (result) {
                        $('.overlay-loading').show();
                        $.ajax({
                            url: '<?php echo base_url("master/service/medic/delete"); ?>',
                            type: "post",
                            dataType: "json",
                            data: {
                                id: id
                            ...
                            success: function(response) {
                                ...
                                if (response.result == 200) {
                                    $('#tableMedicLists').DataTable().ajax.reload()
                                    show_notif('success', response.data.name);
                                }
                            }
                        });
                    } else {
                        show_notif('info', 'Tindakan medikal batal dihapus');
                    }
                }
            });
        }        
